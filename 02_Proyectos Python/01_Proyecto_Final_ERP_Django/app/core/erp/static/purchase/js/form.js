...
var compras = {
    ...
    calculate_invoice: function () {
        ...
        $.each(this.items.products, function (pos, dict) {
            dict.pos = pos;
            dict.subtotal = dict.cant * parseFloat(dict.cost);
...
function formatRepo(repo) {
    ...
    var option = $(
        ...
        //'<b>Proveedor:</b> ' + repo.prov.name + '<br>' +
        '<b>Coste:</b> <span class="badge badge-warning">€' + repo.cost + '</span>' +
        '</p>' +
        '</div>' +
        '</div>' +
        '</div>');
...
$(function () {
    ...
    $('.btnRemoveAll').on('click', function () {
        if (compras.items.products.length === 0) return false;
        alert_action('Notificación', '¿Estas seguro de eliminar todos los items de tu detalle?', function () {
            compras.items.products = [];
            compras.list();
        }, function () {
    ...
    $('#tblProducts tbody')
        ...
        .on('change', 'input[name="cant"]', function () {
            ...
            compras.calculate_invoice();
            $('td:eq(5)', tblProducts.row(tr.row).node()).html('€' + compras.items.products[tr.row].subtotal.toFixed(2));
        });
    ...
    $('form').on('submit', function (e) {
        ...
        submit_with_ajax(window.location.pathname, 'Notificación',
            '¿Estas seguro de realizar la siguiente acción?', parameters, function (response) {
                alert_action('Notificación', '¿Desea imprimir el detalle de compra?', function () {
